stretcheslist<- function(path, windowSize, thr)
{
  ...
  if (length(stretch) != 0){ 
    ...
    #idTable<-'/home/helena/Documentos/Master/IRB/data/CCDS.20180614.txt' #reference table
    idTable <- '/mnt/lribas/users/arafels/irbPhd/4_allData/CDS/eukarya/humanCcds/June2018stable/CCDS.20180614.txt'
    id<-read.table(idTable, sep='\t', header=T) #read table
    genn<-sapply(ccdsId ,function(x) id[id$ccds_id%in%x,'gene'][1])
    ...
    t1 <- data.frame(name,size,a,d,d_a,sizeGen) #create data frame
    colnames(t1)<- c("name","size", "a", "d", "d_a", "sizeGen")
    o<-order(t1$size,t1$d_a, decreasing=T)
    write.csv(t1[o,], 'strGenes_P2.csv', row.names=F) #write csv
    x<-0
  } else {
  ...
  if (length(s1) !=0){ #hi ha gens amb stretches
    ...
    if (length(na4)!=0){
      ...
      ta2<-cbind(nas,windowSize, a[na5], d[na5], d[na5]/a[na5],si[na5])
      colnames(ta2)<-c('name','size','a','d','d_a', 'sizeGen')
      o <- order(as.numeric(ta2[,'d_a']), decreasing=T)
      if (nrow(ta2) == 1){ #si hi ha nomes 1 dona problemes amb el order
        write.table(ta2, 'allGenes_P1.csv', row.names=F)
      }else{
        write.table(ta2[o,], 'allGenes_P1.csv', row.names=F)
      }
      y<-0
    }else {
  ...
  if (y !=1){ #hi ha sense stretches
    p1<-read.table('allGenes_P1.csv', header=T)
    Nas <- rep(NA, length(p1))
  }
  
  if(x!=1){ #hi ha streches
    p2<-read.csv('strGenes_P2.csv', header=T)
  ...
  if (tot == 0){ #tinc els 2 fitxers
    ...
    write.table(pun2, 'allStrUnique.csv', row.names=F)
    ...
    uniprot <- sub(".*\\..*\\..*\\..*\\.(.*?)\\..*", "\\1", aux)
    uniprot
    df <- data.frame(name=pun[,1],gname,id,uniprot,hs,chr,pun[,2:6])
    write.table(df, 'allStr_2.csv', row.names=F)
  }else {
